<?php
    session_start();
    //print_r($_POST);
    //print_r($_SESSION);

    //initializes some session variables
    $_SESSION['prereq1']='';
    $_SESSION['prereq2']='';
    $_SESSION['prereq3']='';
    $_SESSION['prereq4']='';

    //sets posted values into sessions
    //$_SESSION['student'] = $_POST['studentYesNo'];
    //$_SESSION['sid'] = $_POST['SID'];
    
?>
<head>
    <title>BAS Application</title>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
    <script src="http://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/js/bootstrap.min.js"></script>
    <link rel="stylesheet" type="text/css" href="css.css">
    <link rel="stylesheet" href="css/bootstrap.css">
    <link rel="stylesheet" href="css/bootstrap-theme.css">

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="http://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/css/bootstrap.min.css">
          <style>.error{color: #ff0000;}</style>
        
</head>
<body>
    <div class="panel panel-success" id="main-body">
        <div class="panel-body">    
            <div class="container">
                <div id="center">
                        <h2>Networking Prerequisities</h2>
                        <h3>Please check all that you have completed.</h3>
                        <p>NOTE: If you don"t have all of the prerequisites, or have extensive
                        industry experience, an advisor will contact
                        you to discuss options</p>
                    <form method="post" action="education.php" >
                        <input type="checkbox" id="class" name="req1" value="req1">IT 210 or CCENT<br/>
                        <input type="checkbox" id="class" name="req2" value="req2">IT 160 or MTA<br/>
                        <input type="checkbox" id="class" name="req3" value="req3">IT 190 or LPI1 or Linux Essentials<br/>
                        <input type="checkbox" id="class" name="req4" value="req4">IT 102 or a Programming Course<br/><br/>
                                    <input type="checkbox" id="class" name="prereq1" value="" class="hideThis">
                                    <input type="checkbox" id="class" name="prereq2" value="" class="hideThis">
                                    <input type="checkbox" id="class" name="prereq3" value="" class="hideThis">
                                    <input type="checkbox" id="class" name="prereq4" value="" class="hideThis">
                    
                        <label for="notes">Commments:</label><br/>
                    <textarea rows="5" cols="50" name="notes"></textarea>
                    <div> <br>
                        <a href="javascript:history.back()"><input type="button" id="back" value="Previous" class="btn btn-success"></a>
                        <input class = "btn btn-success" input type="submit" name="submit" value="Continue">
                    </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</body>